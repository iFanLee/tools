...
type unicastPolicyAndroid struct {
...
*/
func UnicastJSONByAndroid(deviceToken,msgType,bodyTicker,bodyTitle,bodyText,afterOpen,goUrl,activity string,custom interface{},payloadData map[string]interface{},desc string) string  {
	filter := make(map[string]interface{})
	payload := make(map[string]interface{})
	payload["display_type"] = msgType
	payloadbody := unicastPayloadBodyAndroid{
		Ticker:      bodyTicker,
		Title:       bodyTitle,
		Text:        bodyText,
		Icon:        "",
	...
	payloadExtra := make(map[string]interface{})
	for k,v := range payloadData{
		payloadExtra[k] = v
	}
	payload["extra"] = payloadExtra
	policy := unicastPolicyAndroid{
		StartTime:      "",
		ExpireTime:     "",
		MaxSendNum:     0,
		OutBizNo:       "",
